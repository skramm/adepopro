...
public:
	...
	void printTabulated( std::ostream& f, int tab_size ) const
	{
...
openFile( std::string fn, std::string text )
{
	...
	file << text
		<< "\n# generated by: AdePoPro, see https://github.com/skramm/adepopro"
		<< "\n# generated on: ";
...
printCurrentElement( std::ofstream& file, const TripletMap& elem1, std::string elemStr )
{
	...
	{
		file << "  - " << elemStr << ": " << elem2.first << ": ";
		elem2.second.printTabulated( file, 6 );
		file << '\n';
		sum += elem2.second;
	}
	file << "- TOTAL: ";
	printChars( file, ' ', 15 );
	sum.printTabulated( file, 6 );
	file << "\n\n";
	return sum;
}
//-------------------------------------------------------------------
